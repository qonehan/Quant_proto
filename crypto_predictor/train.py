"""
비트코인 기울기 예측 모델 학습 및 평가 파이프라인.

흐름:
  1. 데이터 로드 (Binance + 온체인 + 센티먼트)
  2. 피처 엔지니어링
  3. 타겟(slope) 생성
  4. Train / Val / Test 분할
...
def train(
    params: HyperParams,
    save_dir: str | pathlib.Path = "checkpoints_crypto",
    data_df=None,
) -> dict:
    """전체 학습 파이프라인을 실행한다.
    ...
    Args:
        ...
        data_df: 외부 데이터프레임 (None이면 API에서 로드)
    ...
    if data_df is None:
        ...
        df = data_df
    logger.info("원본 데이터: %d rows x %d columns", *df.shape)
