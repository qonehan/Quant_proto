"""
학습된 모델을 사용한 비트코인 기울기 추론 모듈.

사용법:
  predictor = SlopePredictor.from_checkpoint("checkpoints_crypto/best_model.pt")
  slope = predictor.predict(latest_data_df)
"""
...
class SlopePredictor:
    ...
    def predict(self, data_df: pd.DataFrame) -> float:
        """최근 데이터로부터 기울기를 예측한다.

        Args:
            data_df: 통합 데이터 DataFrame (OHLCV + 보조 지표).
                     최소 (n + warmup) 행 필요.

        Returns:
            예측된 기울기(slope) 값.
            양수 → 상승, 음수 → 하락, 0에 가까움 → 횡보
        """
        features = build_features(data_df, self.params)
    ...
    def predict_batch(self, data_df: pd.DataFrame) -> np.ndarray:
